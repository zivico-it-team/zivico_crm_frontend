...
const LeaveMobileCard = ({ leave, onApprove, onReject, canManageActions = true }) => {
  return (
    <div className="p-4 space-y-3 bg-white">
      ...
      {/* Leave Balances */}
      {leave.userLeaveBalances && (
        <div className="text-sm text-gray-600">
          <span className="text-xs text-gray-500">Available Leave Balances:</span>
          <div className="mt-1 grid grid-cols-2 gap-2">
            {Object.entries(leave.userLeaveBalances).map(([type, balance]) => (
              <div key={type} className="p-2 rounded bg-blue-50">
                <span className="text-xs text-blue-600 capitalize">{type.replace('_', ' ')}</span>
                <p className="font-medium text-blue-800">{balance} days</p>
              </div>
            ))}
          </div>
        </div>
